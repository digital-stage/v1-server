...
class SocketUserHandler {
  ...
  handle(socket: ITeckosSocket, token: string, initialDevice: Partial<Device>) {
    return this._authentication.verifyWithToken(token).then((user) => {
      ...
      MongoRealtimeDatabase.sendToDevice(
        socket,
        ServerUserEvents.USER_READY,
        user
      );

      return Promise.all([
        deviceHandler
          .generateDevice(initialDevice)
        ...
        .then(() => {
          ...
          return MongoRealtimeDatabase.sendToDevice(
            socket,
            ServerGlobalEvents.READY
          );
        })
