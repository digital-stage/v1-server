...
class SocketHandler {
    ...
    constructor(serverAddress, database: MongoRealtimeDatabase, authentication: IAuthentication, io: socketIO.Server) {
        ...
        this._io.on("connection", (socket: socketIO.Socket) => {
            ...
            return this._authentication.authorizeSocket(socket)
                .then(async (user: User) => {
                    ...
                    const deviceHandler = new SocketDeviceHandler(this._serverAddress, this._database, user, socket);
                    ...
                    this._database.sendToDevice(socket, ServerUserEvents.USER_READY, user);
                    ...
                    return Promise.all([
                        ...
                        .then(() => {
                            ...
                            return socket.join(user._id.toString(), err => {
                                if (err)
                                ...
                                this._database.sendToDevice(socket, ServerGlobalEvents.READY);
                            });
                        })
                        .catch((error) => {
