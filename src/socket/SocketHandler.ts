...
class SocketHandler {
  ...
  constructor(
    ...
    this._userHandler = new SocketUserHandler(serverAddress, database, authentication);
  ...
  private handleConnection = (socket: ITeckosSocket) => {
    socket.on('router', (payload: RouterConnectionPayload) => {
      ...
      if (apiKey) {
        ...
        if (apiKey === API_KEY) {
          return this._routerHandler.handle(socket, router)
            .catch(() => {
              socket.disconnect();
            });
        }
    ...
    socket.on('token', (payload: UserConnectionPayload) => {
      ...
      if (token) {
        return this._userHandler.handle(socket, token, device)
          .catch(() => {
            socket.disconnect();
          });
      }
